...
class RiverPumpedSource(Source):
    ...
    def eq_pump_curve(self):
        ...
        :return:
        """
        self.stnPump = self.STN("stnPump")

        self.STATE("Static")

        eq = self.CreateEquation("WFix")
        eq.Residual = self.w() - self.w0()

        self.STATE("Dynamic")

        self.IF(Time() < 0.01 * Constant(1 * s), eventTolerance=1E-1)
